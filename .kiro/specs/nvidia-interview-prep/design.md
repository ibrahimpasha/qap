# Design Document: NVIDIA Interview Prep Notebooks

## Overview

This feature produces four Jupyter notebooks (.ipynb files) that serve as a self-contained interview preparation course for the NVIDIA Senior Software SDET Test Development Engineer HackerRank assessment. Each notebook maps to one timed section of the assessment: DSA (MCQ, 15 min), OS/Linux (MCQ, 25 min), Python coding (30 min), and REST API coding (25 min).
...
4. **Single output directory**: All four notebooks land in `nvidia_interview_prep/`.
...
sequenceDiagram
    ...
    Gen->>FS: write .ipynb to nvidia_interview_prep/
```
...
def main():
    ...
    for spec in [dsa_spec, os_linux_spec, python_spec, rest_api_spec]:
        ...
        generator.write(notebook, "nvidia_interview_prep", spec.filename)
```
...
- **File output tests**: Verify exactly 4 files are created with correct names in `nvidia_interview_prep/` (Requirements 1.1, 1.2, 1.3)
...
- **OS/Linux content tests**: Verify specific topics and NVIDIA-relevant topics exist, MCQ counts >= 8 per level (Requirements 5.1, 5.2, 5.3, 5.4)
- **Python content tests**: Verify specific topics, NVIDIA examples, problem counts >= 4 per level, timing tips (Requirements 6.1, 6.2, 6.3, 6.5, 6.7)
- **REST API content tests**: Verify specific topics, NVIDIA examples, problem counts >= 4 per level, reference section (Requirements 7.1, 7.2, 7.3, 7.5, 7.7)
...
- **Feature: nvidia-interview-prep, Property 1: Notebook top-level cell ordering** — Generate random NotebookSpecs, verify cell order invariant
- **Feature: nvidia-interview-prep, Property 2: Difficulty ordering and labeling** — Generate notebooks with random topic sections, verify beginner-before-mid-level ordering and labels
- **Feature: nvidia-interview-prep, Property 3: Topic section structure** — Generate random TopicSections, verify each contains explanation → examples → practice → takeaways
- **Feature: nvidia-interview-prep, Property 4: MCQ structural validity** — Generate random MCQs, verify 4 options A-D, explanation presence, code snippet for code_output type
- **Feature: nvidia-interview-prep, Property 5: Solution separation** — Generate random MCQs and practice problems, verify solution cells follow question/problem cells
- **Feature: nvidia-interview-prep, Property 6: Practice problem test validation** — Generate random PracticeProblems, verify test cells contain assert statements
- **Feature: nvidia-interview-prep, Property 7: API problems use mock or public data** — Generate random REST API problems, verify no proprietary API references
- **Feature: nvidia-interview-prep, Property 8: Linux command sections include example output** — Generate random OS/Linux command sections, verify example output presence
- **Feature: nvidia-interview-prep, Property 9: DSA sections include implementation and complexity** — Generate random DSA sections, verify code implementation and complexity analysis
- **Feature: nvidia-interview-prep, Property 10: NotebookSpec round-trip integrity** — Generate random NotebookSpecs, generate notebook, validate with nbformat
...
- Each property test tagged with comment: `# Feature: nvidia-interview-prep, Property N: <title>`
